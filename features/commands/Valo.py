...
class ValoService:
    ...
    async def get_match_history(self, session: aiohttp.ClientSession, puuid: str) -> List[Dict]:
        ...
        for match in matches_data['history'][:5]:  # 최근 5게임만
            match_detail_url = f"{self.request.val_url}/val/match/v1/matches/{match['matchId']}"
            async with session.get(match_detail_url, headers=self.request.headers) as response:
                if response.status == 200:
    ...
    async def get_rank_info(self, session: aiohttp.ClientSession, puuid: str) -> tuple[Dict, str]:
...
class Valo(commands.Cog):
    ...
    @commands.command(name="발로.티어", aliases=['발로.랭크'], description="현재 시즌 티어")
