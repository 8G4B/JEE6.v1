...
from src.config.settings.gambling_settings import (
    MIN_BET, MAX_BET, GAME_COOLDOWN, GAME_MULTIPLIER_RANGES
)
...
class GamblingGames(BaseCommand):
    ...
    async def _play_game(self, ctx, user_id: int, server_id: int, 
                         guess: str, result: str, bet: int, game_type: str) -> discord.Embed:
        ...
        try:
            async with lock:
                ...
                if is_correct:
                    ...
                    await self.gambling_service.subtract_balance(user_id, server_id, bet)
                    balance = await self.gambling_service.get_balance(user_id, server_id)
                    return GamblingEmbed.create_game_embed(
                        author_name=ctx.author.name,
                        is_correct=False,
    ...
    async def coin(self, ctx, bet: str = None):
        ...
        try:
            remaining = await self.gambling_service.check_cooldown(user_id, "coin", GAME_COOLDOWN)
            if remaining:
            ...
            await ctx.reply(embed=GamblingEmbed.create_error_embed(f"오류가 발생했습니다: {e}"))
        finally:
    ...
    async def dice(self, ctx, bet: str = None):
        ...
        try:
            remaining = await self.gambling_service.check_cooldown(user_id, "dice", GAME_COOLDOWN)
            if remaining:
                await ctx.reply(embed=GamblingEmbed.create_cooldown_embed(remaining))
                return
            bet_amount = await self._parse_bet_amount(bet, user_id, server_id)
