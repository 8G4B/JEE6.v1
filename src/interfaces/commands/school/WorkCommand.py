...
from src.services.WorkService import WorkMode, WorkService
...
class WorkCommand(BaseCommand):
    ...
    def _start_countdown(self, channel_id: int, message, status) -> None:
        ...
        task = asyncio.create_task(
            ...
            name=f"work-countdown-{channel_id}",
        )
    ...
    async def _update_countdown(self, message, status) -> None:
        ...
        try:
            while status.state == "countdown":
                await asyncio.sleep(max(0, next_update - loop.time()))
                now = self._now(status)
                if now >= status.target:
                    status = self.work_service.get_status(status.mode, now)
                else:
                    status = replace(status, now=now)
                await message.edit(embed=WorkEmbed.create_work_embed(status))
            ...
            logger.info("퇴근 카운트다운 메시지가 없어 갱신을 종료합니다.")
        except discord.HTTPException:
            logger.warning(
                "퇴근 카운트다운 메시지 갱신에 실패했습니다.",
                exc_info=True,
            )
        except Exception:
            logger.exception("퇴근 카운트다운 갱신 중 오류가 발생했습니다.")

    async def _reply(self, ctx, mode: WorkMode) -> None:
        try:
            status = self.work_service.get_status(mode)
            message = await ctx.reply(embed=WorkEmbed.create_work_embed(status))
            if status.state == "countdown":
                self._start_countdown(ctx.channel.id, message, status)
            else:
                self._stop_countdown(ctx.channel.id)
        except Exception:
            logger.exception("퇴근 시간 계산 중 오류가 발생했습니다.")
            await ctx.reply(embed=WorkEmbed.create_error_embed())

    @commands.command(
        name="퇴근",
        description="매일 오후 4시 30분까지 남은 시간을 확인합니다.",
    )
    async def leave_work(self, ctx):
        await self._reply(ctx, "regular")

    @commands.command(
        name="야근",
        description="매일 오후 9시 30분까지 남은 시간을 확인합니다.",
    )
    async def overtime(self, ctx):
        await self._reply(ctx, "overtime")
