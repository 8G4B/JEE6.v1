...
class GamblingCardGames(BaseCommand):
    ...
    async def blackjack(self, ctx, bet: str = None):
        ...
        remaining = await self.gambling_service.check_cooldown(user_id, "blackjack", GAME_COOLDOWN)
        if remaining:
    ...
    async def baccarat(self, ctx, bet: str = None):
        ...
        remaining = await self.gambling_service.check_cooldown(user_id, "baccarat", GAME_COOLDOWN)
        if remaining:
            await ctx.reply(embed=GamblingEmbed.create_cooldown_embed(remaining))
            return
    ...
    async def indian_poker(self, ctx, bet: str = None):
        ...
        remaining = await self.gambling_service.check_cooldown(user_id, "indian_poker", GAME_COOLDOWN)
        if remaining:
            await ctx.reply(embed=GamblingEmbed.create_cooldown_embed(remaining))
            return
