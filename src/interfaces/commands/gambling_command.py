...
class GamblingCommands(BaseCommand):
    ...
    async def _parse_bet_amount(self, bet_str: str, user_id: int, server_id: int) -> int:
        if bet_str == "올인":
    ...
    async def work(self, ctx):
        ...
        try:
            remaining = await self.gambling_service.check_cooldown(user_id, "work", WORK_COOLDOWN)
            if remaining:
            ...
            async with lock:
                ...
                balance = await self.gambling_service.get_balance(user_id, server_id)
                embed = GamblingEmbed.create_work_embed(ctx.author.name, amount, balance)
                await ctx.reply(embed=embed)
    ...
    async def ranking(self, ctx):
        ...
        async with ctx.typing():
            rankings = await self.gambling_service.get_cached_rankings(server_id, self.bot, 3)
            top_3 = rankings[:3]
    ...
    async def all_ranking(self, ctx):
        ...
        async with ctx.typing():
            rankings = await self.gambling_service.get_cached_rankings(server_id, self.bot, 100)
            
            if not rankings:
                embed = GamblingEmbed.create_ranking_embed("🏅 전체 랭킹", "랭킹 정보가 없습니다.")
                await ctx.reply(embed=embed)
                return
            ...
            current_page = 0
            embed = GamblingEmbed.create_ranking_embed("🏅 전체 랭킹", pages[current_page])
    ...
    async def transfer(self, ctx, recipient: discord.Member = None, amount: str = None):
        ...
        if sender_id == recipient_id:
            await ctx.reply(embed=GamblingEmbed.create_error_embed("자기 자신에게는 송금할 수 없습니다."))
            return
        ...
        if amount_value <= MIN_JACKPOT_BET:
            await ctx.reply(embed=GamblingEmbed.create_error_embed(f"{MIN_JACKPOT_BET:,}원 이하는 송금할 수 없습니다."))
            return

        if amount_value >= MAX_BET:
            await ctx.reply(embed=GamblingEmbed.create_error_embed(f"{MAX_BET:,}원 이상 송금할 수 없습니다"))
            return
        ...
        async with sender_lock:
            async with recipient_lock:
                sender_balance = await self.gambling_service.get_balance(sender_id, server_id)
                ...
                await self.gambling_service.subtract_balance(sender_id, server_id, amount_value)
                await self.gambling_service.add_balance(recipient_id, server_id, amount_after_tax)
    ...
    async def jackpot(self, ctx, bet: str = None):
        ...
        try:
            remaining = await self.gambling_service.check_cooldown(user_id, "jackpot", GAME_COOLDOWN)
            if remaining:
                await ctx.reply(embed=GamblingEmbed.create_cooldown_embed(remaining))
                return
            
            remaining = await self.gambling_service.check_cooldown(user_id, "jackpot_win", JACKPOT_WIN_COOLDOWN)
            if remaining:
                minutes = remaining // 60
                seconds = remaining % 60
                time_str = f"{minutes}분 {seconds}초" if minutes > 0 else f"{seconds}초"
                await ctx.reply(embed=GamblingEmbed.create_error_embed(f"{time_str} 후에 다시 시도해주세요."))
                return

            bet_amount = await self._parse_bet_amount(bet, user_id, server_id)
            
            if bet_amount is None or bet_amount < MIN_JACKPOT_BET:
                await ctx.reply(embed=GamblingEmbed.create_error_embed(f"{MIN_JACKPOT_BET:,}원 이상 베팅하세요"))
                return

            if bet_amount >= MAX_BET:
                await ctx.reply(embed=GamblingEmbed.create_error_embed(f"{MAX_BET:,}원 이상 베팅할 수 없습니다"))
                return

            lock = await self.gambling_service._get_lock(user_id)
            async with lock:
                current_balance = await self.gambling_service.get_balance(user_id, server_id)
    ...
    async def cog_check(self, ctx):
        ...
        if (user_id in self.blackjack_players or 
            user_id in self.baccarat_players or 
            user_id in self.indian_poker_players or 
            user_id in self.coin_players or 
            user_id in self.dice_players):
