...
class JusticeRepository(RawRepositoryBase):
    ...
    async def set_user_count(self, user_id: int, server_id: int, count: int) -> bool:
        ...
        try:
            ...
            self.log_operation(f"set_user_count", f"{user_id}, {server_id}, {count}", success=success)
            return success
        except Exception as e:
    ...
    async def add_timeout_history(self, history: TimeoutHistory) -> bool:
        ...
        try:
            ...
            success = result is True
            self.log_operation(f"add_timeout_history", f"{history.user_id}, {history.server_id}, {history.moderator_id}", success=success)
            return success
        except Exception as e:
