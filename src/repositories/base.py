    ...
    def update(self, id: Any, **kwargs) -> bool:
        try:
            with get_db_session() as session:
                instance = session.query(self.model).get(id)
                if not instance:
                    logger.warning(f"업데이트 실패: {self.model.__name__}(id={id}) - 항목이 없습니다")
                    return False
            ...
            logger.error(f"업데이트 실패: {self.model.__name__}(id={id}, {kwargs}) - {e}")
            return False
    ...
    def delete(self, id: Any) -> bool:
        try:
            with get_db_session() as session:
                instance = session.query(self.model).get(id)
                if not instance:
                    logger.warning(f"삭제 실패: {self.model.__name__}(id={id}) - 항목이 없습니다")
                    return False
