...
class GamblingService:
    ...
    def validate_bet(self, bet_amount: Optional[int], min_bet: int = MIN_BET, max_bet: int = MAX_BET) -> Optional[str]:
    ...
    async def check_cooldown(self, user_id: int, action_type: str, cooldown_seconds: int) -> int:
    ...
    def calculate_tax(self, amount: int, tax_type: str = "income") -> int:
        if tax_type == "securities" or tax_type in ["coin", "dice", "blackjack", "baccarat", "indian_poker"]:
    ...
    async def get_cached_rankings(self, server_id: int, bot, limit: int = 10) -> List[Tuple[int, str, int]]:
        current_time = time.time()
        
        if (server_id in self._rankings_cache and 
            server_id in self._rankings_cache_time and
            current_time - self._rankings_cache_time[server_id] < self.CACHE_EXPIRATION):
