...
from src.config.settings.meal_settings import (
    MEAL_API_KEY, ATPT_OFCDC_SC_CODE, SD_SCHUL_CODE,
    NO_MEAL, MEAL_TIMES, CACHE_DURATION
)
...
class MealService:
                ...
                async with session.get(self.base_url, params=params) as response:
                    ...
                    if 'mealServiceDietInfo' in data:
                        ...
                        for meal in meals:  
                            meal['DDISH_NM'] = '\n'.join(
                                f'- {dish.strip()}' 
                                for dish in meal['DDISH_NM'].replace('*', '').split('<br/>') 
                                if dish.strip()
                            )
    ...
    async def _get_menu_from_meal_info(self, meal_info: list, meal_code: str) -> str:
        return next(
            (meal["DDISH_NM"] for meal in meal_info if meal["MMEAL_SC_CODE"] == meal_code), 
            NO_MEAL
        )
    
    async def get_current_meal(self, now: datetime) -> Tuple[Optional[str], Optional[str]]:
    ...
    async def get_meal_by_type(self, date: str, meal_code: str, title: str) -> Tuple[Optional[str], Optional[str]]:
