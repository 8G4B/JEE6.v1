...
from src.config.settings.riot_settings import (
    RIOT_API_KEY, LOL_BASE_URL, LOL_ASIA_URL, 
    RIOT_HEADERS, LOL_GAME_MODES
)
...
class LolService:
    ...
    async def get_tier_info(self, session: aiohttp.ClientSession, puuid: str) -> Tuple[Optional[Dict], str]:
    ...
    async def get_match_history(self, session: aiohttp.ClientSession, puuid: str) -> List[Dict]:
        ...
        try:
            ...
            match_data_list = [data for data in match_data_list if data is not None and not isinstance(data, Exception)]
        ...
        try:
            async with session.get(url, headers=self.headers) as response:
                if response.status == 200:
                    ...
                    return data
                logger.warning(f"매치 정보 요청 실패: {match_id}, 상태 코드: {response.status}")
                return None
        except Exception as e:
