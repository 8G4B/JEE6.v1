...
class GamblingService:
    ...
    def validate_bet(
        self, bet: Optional[int], user_id: Optional[int] = None
    ) -> Optional[discord.Embed]:
        if bet is None:
            return GamblingEmbed.create_error_embed("베팅 금액을 입력해주세요.")
        if bet < GamblingConfig.MIN_BET:
            return GamblingEmbed.create_error_embed(
                f"최소 {GamblingConfig.MIN_BET:,}원 이상 베팅해주세요."
            )
        if bet > GamblingConfig.MAX_BET:
            return GamblingEmbed.create_error_embed(
                f"최대 {GamblingConfig.MAX_BET:,}원까지 베팅할 수 있어요."
            )
        return None
    ...
    async def get_cached_rankings(self, bot) -> List[Tuple[int, str, int]]:
        ...
        if (
            self._rankings_cache is not None
            and self._last_rankings_update is not None
            and current_time - self._last_rankings_update < self._rankings_cache_ttl
        ):
...
class Gambling(commands.Cog):
            ...
            async with lock:
                ...
                if is_correct:
                    multiplier = random.uniform(
                        *GamblingConfig.GAME_MULTIPLIER_RANGES[game_type]
                    )
    ...
    async def coin(self, ctx, bet: str = None):
        ...
        try:
            bet = (
                int(bet)
                if bet != "올인"
                else await self.gambling_service.get_balance(ctx.author.id)
            )
        ...
        try:
            ...
            embed = await self._play_game(
                ctx.author.id, ctx.author.name, guess, result, bet, "coin"
            )
    ...
    async def dice(self, ctx, bet: str = None):
        ...
        try:
            ...
            embed = await self._play_game(
                ctx.author.id, ctx.author.name, guess, result, bet, "dice"
            )
            await game_message.edit(embed=embed)
    ...
    async def jackpot(self, ctx, bet: str = None):
        ...
        if bet is None or bet < GamblingConfig.MIN_JACKPOT_BET:
            await ctx.reply(
                embed=GamblingEmbed.create_error_embed("1,000원 이상 베팅하세요")
            )
            self.game_manager.end_game(ctx.author.id)
            return
        ...
        async with lock:
            ...
            if secrets.randbelow(100) <= 1:  # 1% 확률
                ...
                winnings_after_tax = winnings - tax
                await self.gambling_service.add_balance(
                    ctx.author.id, winnings_after_tax
                )
    ...
    async def work(self, ctx):
        ...
        async with lock:
            ...
            if (
                last_used
                and (current_time - last_used).total_seconds()
                < GamblingConfig.WORK_COOLDOWN
            ):
                remaining = GamblingConfig.WORK_COOLDOWN - int(
                    (current_time - last_used).total_seconds()
                )
                embed = discord.Embed(
                    title="힘들어서 쉬는 중 ㅋ",
            ...
            while True:
                try:
                    ...
                    if str(reaction.emoji) == "👊":
                        ...
                        embed.add_field(
                            name="선택", value="👊 Hit 또는 🛑 Stand", inline=False
                        )
                        await game_message.edit(embed=embed)
                            ...
                            if dealer_value > 21 or player_value > dealer_value:
                                multiplier = (
                                    ...
                                    if player_value == 21
                                        ...
                                        *GamblingConfig.GAME_MULTIPLIER_RANGES[
                                            "blackjack"
                                        ]
                                    )
                                )
    ...
    async def baccarat(self, ctx, bet: str = None):
        ...
        try:
            ...
            embed.add_field(
                name="선택",
                value=f"👤 Player: {ctx.author.name} / 🏦 Banker: JEE6 / 🤝 Tie",
                inline=False,
            )
                    ...
                    if guess == result:
                        multiplier = (
                            ...
                            if result == "Tie"
                                ...
                                *GamblingConfig.GAME_MULTIPLIER_RANGES["baccarat"]
                            )
                        )
                        winnings = int(bet * multiplier)
                        ...
                        winnings_after_tax = winnings - tax
                        await self.gambling_service.add_balance(
                            ctx.author.id, winnings_after_tax - bet
                        )
                ...
                with await self.gambling_service._get_lock(ctx.author.id):
                    if str(reaction.emoji) == "💀":  # 다이
                        ...
                        if player_card > banker_card:
                            multiplier = random.uniform(
                                *GamblingConfig.GAME_MULTIPLIER_RANGES["indian_poker"]
                            )
                            winnings = int(bet * multiplier)
                            tax = self.gambling_service.calculate_tax(
    ...
    async def cog_check(self, ctx):
        if ctx.author.id in (
            ...
            | self.baccarat_players
            | self.indian_poker_players
            | self.coin_players
            | self.dice_players
        ):
