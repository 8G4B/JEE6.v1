...
class LolService:
    ...
    async def get_tier_info(
        ...
        summoner_url = (
            f"{self.request.base_url}/lol/summoner/v4/summoners/by-puuid/{puuid}"
        )
    ...
    async def get_match_history(
        ...
        async def fetch_matches():
            async with session.get(
                matches_url, headers=self.request.headers
            ) as response:
                if response.status != 200:
                    raise ValueError(response.status)
        ...
        for match_data in match_data_list:
            ...
            champion_name = self.request.get_champion_name_kr(
                self.champions_data, champion_id
            )
...
class Lol(commands.Cog):
    ...
    @commands.command(
        name="롤.로테이션", description="현재 무료 로테이션 챔피언 목록을 보여줍니다"
    )
