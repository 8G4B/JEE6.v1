...
class GPTService:
    ...
    def get_answer(self, user: str) -> str:
        return self.gpt_request.send_request(
            "gpt-3.5-turbo", "다음 질문에 대해 간단히 답변해주세요.", user
        )
...
class QuestionEmbed:
    ...
    def create_answer_embed(answer: str, question: str) -> discord.Embed:
        embed = discord.Embed(
            title="답변", description=answer, color=discord.Color.green()
        )
...
class Question(commands.Cog):
    ...
    async def question(self, ctx, *, question=None):
        if question is None:
            embed = QuestionEmbed.create_error_embed(
                "!질문 [질문할 내용] <-- 이렇게 써"
            )
